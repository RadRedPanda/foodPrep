				...
				for(day = 1; day < 8; day++){
					for(hour = 0; hour < obj[day-1].length; hour++){
						...
						if(foodPrepArray[obj[day-1][hour].time][day-1].length > 1){
							...
							tableText += '<select onchange="hoverFood(' + day.toString()+ ', ' + obj[day-1][hour].time.toString() + ', this.selectedIndex);" style="width: 84px" id="' + day.toString() + ',' + obj[day-1][hour].time.toString() + 'select"><div class="truncated"><option>' + obj[day-1][hour].name + '</option></select>';
